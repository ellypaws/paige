...
type Parameters struct {
	...
	Img2Img             *Img2ImgParams `json:"img2img,omitempty"` // used by Img2Img
	Image               *async         `json:"image,omitempty"`   // used by Img2Img and Inpainting
	...
	// Director tools
	DirectorReferenceDescriptions            []DirectorDescription `json:"director_reference_descriptions,omitempty"`
	DirectorReferenceImages                  []string              `json:"director_reference_images,omitempty"` // Base64 strings
	DirectorReferenceInformationExtracted    []float64             `json:"director_reference_information_extracted,omitempty"`
...
type Img2ImgParams struct {
	ColorCorrect   bool    `json:"color_correct"`
	ExtraNoiseSeed int64   `json:"extra_noise_seed"`
	Noise          float64 `json:"noise"`
	Strength       float64 `json:"strength"`
}

type DirectorDescription struct {
	Caption   V4Prompt `json:"caption"`
...
func (r *NovelAIRequest) Init() {
	...
	case ModelV45Full, ModelV45Curated, ModelV4Full, ModelV4Preview:
		existingChars := r.Parameters.V4Prompt.Caption.CharCaptions
		if existingChars == nil {
			existingChars = make([]CharCaption, 0)
		}
		baseCaption := r.Parameters.V4Prompt.Caption.BaseCaption
		if baseCaption == "" {
		...
		r.Parameters.V4Prompt = V4Prompt{
			Caption: Caption{
				...
				CharCaptions: existingChars,
			},
			UseCoords: false,
			UseOrder:  true,
		}

		existingNegChars := r.Parameters.V4NegativePrompt.Caption.CharCaptions
		if existingNegChars == nil {
			existingNegChars = make([]CharCaption, 0)
		}
		negCaption := r.Parameters.V4NegativePrompt.Caption.BaseCaption
		if negCaption == "" {
			negCaption = r.Parameters.NegativePrompt
		}

		r.Parameters.V4NegativePrompt = V4Prompt{
			Caption: Caption{
				BaseCaption:  negCaption,
				CharCaptions: existingNegChars,
			},
			UseCoords: false,
			UseOrder:  true,
		}
...
// SetPrompts configures the request with the given prompts, populating Input, V4Prompt, etc.
func (r *NovelAIRequest) SetPrompts(general string, characters []CharCaption, negative string) {
	// r.Input = general
	// r.Parameters.Prompt = general
	r.Parameters.NegativePrompt = negative
